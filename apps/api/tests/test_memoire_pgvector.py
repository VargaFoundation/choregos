"""La mémoire `pgvector` (le repli sans service de plus) face à une vraie base.
...
def vrais_adaptateurs(monkeypatch: pytest.MonkeyPatch) -> None:
    """`CHOREGOS_FAKES=1` remplacerait pgvector par le fake en mémoire : ici on veut la base."""
...
async def test_ecrit_cherche_propose_et_valide_sur_la_base(client: AsyncClient, org: str) -> None:
    ...
    d'un agent, validation humaine par l'API — qui lit le connecteur `pgvector` du projet."""
...
async def test_sur_postgres_chaque_organisation_ne_voit_que_sa_memoire(pg_app: Any) -> None:
    ...
    nue = PgVectorMemory(get_sessionmaker(), ModelesPgVector(Project=Project, MemoryFact=MemoryFact))
